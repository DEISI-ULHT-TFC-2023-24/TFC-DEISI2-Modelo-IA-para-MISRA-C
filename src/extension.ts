...
context.subscriptions.push(
	...
	vscode.commands.registerCommand('chatgpt.misracrules', () => commandHandler('promptPrefix.misracrules')),
	vscode.commands.registerCommand('chatgpt.resetConversation', () => provider.resetConversation()),
	vscode.commands.registerCommand('chatgpt.addapmisrac', () => {
		const misraCValue = 'Addapt to MISRA C rules and comment every line the rule number';
...
class ChatGPTViewProvider implements vscode.WebviewViewProvider {
	...
	private _newAPI() {
		...
		if (!this._authInfo || !this._settings?.apiUrl) {
			console.warn("API key or API URL not set, please go to extension settings");
		}else{	
